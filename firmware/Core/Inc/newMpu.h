...
#ifndef INC_NEWMPU_H_
#define INC_NEWMPU_H_
...
{
	uint8_t xd; // `data` will store the register data
	spi_enable();
	uint8_t data = subAddress | 0x80;
	HAL_SPI_Transmit(&MPU9250_SPI, &data, 1, 0xFF);
	HAL_SPI_Receive(&MPU9250_SPI, &xd, 1, 0xFF);
	spi_disable();
	return xd;
}
...
{
	uint8_t data_to_read[14]; // `data` will store the register data
	spi_enable();
	uint8_t data = subAddress | 0x80;
	HAL_SPI_Transmit(&MPU9250_SPI, &data, 1, 0xFF);
	HAL_SPI_Receive(&MPU9250_SPI, data_to_read, count, 0xFF);
	spi_disable();

	for(int ii = 0; ii < count; ii++) {
		dest[ii] = data_to_read[ii];
	}
}
